...
Арбитр применяет этот же `PATH` override только в project `test_cmd`; не
полагайтесь на совпадение с интерактивным shell владельца. Для `$ python3
scripts/check_environment.py` скрипт сам выбирает канонический интерпретатор.
До появления обычных тестов
